...
3. **Ensuring Template Adaptability**  
   The pipeline aims to support flexible and scalable template extraction and generation, enabling easy adaptation to different bank formats (e.g., Bank of America, Wells Fargo). By improving HTML template analysis and field identification.
...
1. **Data and Template Extraction**  
   ...
   - **Process**: A Chase-specific HTML template (`chase_final.html`) defines the statement’s structure, with placeholders for mutable fields (e.g., account holder, transactions) and immutable fields (e.g., bank name, headers). The template is analyzed to identify fields using regex and LLMs.  
...
## Example Output
Below is a sample synthetic Chase bank statement generated by the pipeline:

![Sample Synthetic Chase Statement](sample_logos/chase-output-image.png)

*Note*: This is a synthetic example and does not represent real financial data.
